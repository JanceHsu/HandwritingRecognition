...
项目不依赖真正的 OpenCV 库。`src/opencv2/` 下是纯 C++ 实现的轻量级 API shim，提供 `cv::Mat`、`cvtColor`、`resize` 等最小集合。实际识别主路径使用 `QImage` 版本的预处理。

详细的推理技术细节参见 [model.md](model.md)，OpenCV 兼容层参见 [opencv.md](opencv.md)。
...
| `src/recognizer.h/cpp` | 识别器：LibTorch 模型加载、图像预处理、推理、设备管理 |
| `src/airwritecontroller.h/cpp` | 隔空书写控制器：Python 进程管理、帧发送、JSON 解析 |
| `src/opencv2/core.hpp` | OpenCV 兼容层：cv::Mat、cv::Size、cv::Scalar |
| `src/opencv2/imgproc.hpp` | OpenCV 兼容层：cvtColor、resize |
