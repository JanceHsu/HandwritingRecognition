...
本项目在传统鼠标画板之外实现了基于摄像头的隔空手势书写。用户伸出食指在空中比划，MediaPipe 实时追踪指尖运动并映射到画布坐标。相比简单的"检测到手指就画"方案，系统设计了六层递进的稳定策略：食指锁定状态机（检测结果异常时沿历史轨迹预测）、限幅平滑（每帧移动距离不超过动态上限）、连续置信度评分（综合食指伸展、其他手指弯曲、指尖锁定信任度输出 0-1 分数）、双路径状态机（手势明确离开书写状态时立即停止，信号抖动时 6 帧后停止）、短时丢手保活（1-2 帧丢失时沿轨迹预测）、Qt 端停笔缓冲（120ms 确认期）。这套机制使得隔空书写在光照不均、手指部分遮挡等条件下仍能保持稳定。

### 7.2 实时可视化反馈

系统在三个层面提供了实时可视化反馈：

- **识别可信度颜色编码**：识别完成后，可信度标签根据数值自动变色（绿色 >= 90%、灰色 70%-90%、红色 < 70%），用户无需阅读数字即可直观判断结果可靠性。
- **隔空书写追踪指标**：摄像头预览下方实时显示书写置信度（红→绿渐变）、手势状态（书写中/缓冲中/空闲，分别用绿/橙/灰色标识）和食指锁定状态（可信绿色/不可信灰色）。
- **摄像头画面叠加标记**：预览画面上叠加食指位置圆圈和 DRAW/TRACK 文字标注，用户可直观看到系统对手指的跟踪结果。

### 7.3 跨语言实时通信架构

隔空书写涉及 Qt C++ 和 Python 两种语言的实时协作。设计了高效的通信方案：Qt 通过 `QProcess` stdin 以长度前缀二进制协议发送压缩 JPEG 帧，Python 通过 stdout 返回轻量 JSON（坐标、状态、置信度）。关键设计决策是图像数据只从 Qt 单向流向 Python，Python 不返回图像，从根本上消除了管道阻塞。Python 端采用三线程架构（采集、检测、主循环），检测线程只取最新帧、丢弃旧帧，保证追踪的实时性。

### 7.4 工程化双构建系统

项目同时维护 CMake 和 qmake 两套构建入口，分别服务于发布流程和开发调试。在这个过程中发现了 CMake `find_package(Torch)` 自动注入的 MSVC 链接器选项 `-INCLUDE:?warp_size@cuda@at@@YAHXZ` 是 CUDA 正常工作的关键——缺少该选项时链接器的死代码消除会移除 CUDA 初始化入口。此外解决了 LibTorch 与 Qt 的 `slots`/`signals` 宏冲突。配套的自动化脚本链（训练→打包→部署→一键启动）使得从代码修改到可分发包的全流程可重复、可检查。

### 7.5 完全离线的隐私保护架构

整个系统在本地运行，不需要网络连接。模型训练、推理、手部检测全部在本地完成。MediaPipe 模型下载一次后缓存在本地，摄像头画面只在本地进程间传递。这使得系统适合对数据隐私有要求的使用场景。

### 7.6 操作日志与可追溯性

程序右侧提供实时操作日志区域，带时间戳记录模型加载、识别结果、摄像头状态、隔空书写启停和异常信息等关键事件，限制最大 300 行防止内存溢出。用户和开发者可以快速了解程序运行流程，在出现问题时无需额外调试工具即可定位原因。
