...
> 当前实现进一步强化了整手骨架判定：手势状态不再只依赖食指单点，而是结合掌心、MCP、PIP、TIP 的相对几何关系来确认“仅食指抬起”，并配合更高频的帧发送与更轻量的平滑，降低误检与显示延迟。
> 在此基础上新增“食指锁定”策略：当当前帧的食指关键点与历史轨迹明显不一致时，短时沿历史速度预测并等待稳定重定位，避免在角度变化时误跳到其他手指；并对短时丢手做保活，减少作画中断。
