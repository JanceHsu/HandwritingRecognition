...
本文档基于项目源码 `scripts/train_mnist.py`（训练脚本）和 `src/recognizer.cpp`（C++ 推理模块）的实际实现，详细说明模型在训练过程、算法策略、优化手段、生成产物以及客户端调用时的全部区别与注意事项。
...
**结论：** 项目使用单一模型文件，不区分训练设备。网络层数、参数量、激活函数、Dropout 比例完全一致。推理设备通过界面复选框在运行时选择，与模型文件无关。
...
**因此，项目只保留一份模型文件**，标准 TorchScript `.pt` 格式。训练设备的不同不影响模型文件的格式和兼容性。
...
C++ 推理模块 `src/recognizer.cpp` 的设备选择由界面层控制：

- 程序启动时默认使用 CPU 推理
- 界面提供"使用 CUDA 进行推理"复选框，勾选后切换为 CUDA 设备
- CUDA 不可用时，复选框被禁用，确保始终使用 CPU

无论推理设备是 CPU 还是 CUDA，都加载同一份 `mnist_model.pt` 模型文件。TorchScript 模型是设备无关的——加载时 LibTorch 会将模型参数放到指定的设备上。
...
在 Qt 主界面中，用户可以通过复选框控制推理设备：

- **未勾选（默认）**：使用 CPU 推理
- **勾选"使用 CUDA 进行推理"**：切换为 CUDA 推理（仅当 CUDA 可用时可勾选）
- **CUDA 不可用时**：复选框被禁用，确保推理设备始终为 CPU

切换设备时，程序会销毁当前识别器实例并以新设备重新加载模型。项目只使用一份模型文件 `mnist_model.pt`，不区分 CPU/GPU 目录。

### 7.6 模型文件查找与回退机制

程序启动时会按以下顺序查找模型文件 `mnist_model.pt`：

1. 搜索路径包括：可执行文件目录、上级目录、`artifacts/models`、`dist/models` 等多个候选位置
2. 如果找不到模型文件，识别功能将被禁用，界面给出提示
...
简单来说，项目使用单一模型文件，推理设备通过界面复选框在运行时选择。CUDA 推理利用了混合精度训练和各种硬件加速技巧，所以训练更快、最终精度略高。但在客户端调用时，CPU 和 CUDA 两种设备的使用方式几乎完全相同，区别仅在于 CUDA 预热和数据传输。
